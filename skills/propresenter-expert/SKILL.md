...
14. **`PUT /v1/playlist/{id}` regrava a lista inteira e o formato NAO é o do `GET`.** No `GET` o `id.uuid` do item é o id do *item* e o `target_uuid` é o id da *apresentação*; o `PUT` procura a apresentação em `id.uuid` (senão **404**). Regra: antes do `PUT`, `id.uuid = target_uuid` (`normalizarItensPlaylist()`). Item de apresentação **sem `target_uuid`** (sem vínculo) dá **400** → gravar como `type:"placeholder"` + `target_uuid:""` (o nome é mantido) e avisar. Mídia cujo arquivo não existe dá **404** sempre → recusar com mensagem clara, sem alterar nada. Sempre fazer backup da lista (`logs\backups`) antes e reler depois.
